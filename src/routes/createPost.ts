...
interface PostData {
  ...
  slug: string
  customslug: boolean
  customSlugData: string
  userSetCreateAt: string
}
...
const postSchema = z.object({
  ...
  userSetCreateAt: z.string().optional()
})
...
export async function createPost(app: FastifyInstance) {
  app.post<{ Body: ZodType<typeof postSchema> }>(
    ...
    async (request, reply) => {
      ...
      const { title, article, category, author, vuecomponent, published, slug, customslug, userSetCreateAt } = request.body as PostData

      // se nao enviar customslug, generate a new from title
